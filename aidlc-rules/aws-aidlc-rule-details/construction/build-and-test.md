...
- **Requirement verification**: Map each implemented requirement, user story,
  acceptance criterion, API contract, and policy to the test evidence that proves it

Rules:
- Build and Test must execute both feature-focused tests and relevant regression tests.
- A command such as `mvn test`, `npm test`, or `.\gradlew.bat test` is execution
  evidence, but the summary must also identify which tests validate the changed
  feature or contract.
- Any implemented requirement or story without automated test evidence must be
  marked N/A with rationale and, when applicable, a manual verification method.
...
- **Feature Test Evidence**: [List test classes or methods that directly verify changed requirements/stories]
- **Regression Evidence**: [List commands or suites proving existing behavior still passes]
...
- **Requirement/Story Coverage**: [List requirements or stories validated by these scenarios]
...
| Requirement/Story | Acceptance Criteria or Contract | Test Evidence | Command | Result |
| --- | --- | --- | --- | --- |
| R-[ID]/S-[ID] | [Observable condition] | [Test class, method, report, or scenario] | [Command executed] | [Pass/Fail/N/A] |

### Verification Notes
- **Feature Tests**: [Tests that directly validate newly implemented or changed behavior]
- **Regression Tests**: [Existing suites or commands that passed]
- **N/A Items**: [Requirements/stories without automated evidence and rationale]
- **Manual Checks**: [Manual verification performed, if any]

## Overall Status
- **Build**: [Success/Failed]
- **All Tests**: [Pass/Fail]
- **Requirement Verification**: [Complete/Incomplete]
- **Ready for Operations**: [Yes/No]

## Next Steps
[If all pass]: Ready to proceed to Operations phase for deployment planning
[If failures or incomplete verification]: Address failing tests or missing
verification evidence and rebuild
```
...
```
     ...
     2. **AI Summary** (optional): Provide structured bullet-point summary of build and test results
        ...
        - List requirement/story verification status and key test evidence
...
**Requirement Verification Status**: [Complete/Incomplete]
**Requirement Verification Evidence**:
- [Requirement/Story] -> [Test evidence] -> [Pass/Fail/N/A]
**Files Generated**:
