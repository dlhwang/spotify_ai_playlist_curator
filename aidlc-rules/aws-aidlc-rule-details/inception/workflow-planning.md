...
- **Requirement verification**: Tests or checks needed to prove each changed
  requirement, story, acceptance criterion, API contract, or policy
...
## Step 5.4: Requirement Verification Planning

Create a requirement verification plan that maps approved requirements and
stories to the test evidence expected during Code Generation and Build and
Test.
...
| --- | --- | --- | --- | --- | --- |
| R-[ID]/S-[ID] | [Observable condition] | [Test or assertion needed] | [unit/integration/contract/e2e/security/performance] | [Path or scenario name] | Pass |
```

Rules:
- 새 기능 또는 동작 변경은 기능을 직접 검증하는 테스트 증거가 있어야 한다.
- 기존 기능 회귀 위험이 있으면 전체 또는 관련 회귀 테스트 명령을 포함한다.
- 자동화하지 않는 항목은 N/A로 두고 사유와 수동 검증 방법을 기록한다.
- 이 계획은 Code Generation 계획의 테스트 작성 항목과 Build and Test
  summary의 requirement verification 결과로 이어져야 한다.
...
- **Quality Gates**: [List]
- **Requirement Verification**: [Requirement/story verification plan, feature
  tests required, and regression commands]
