...
- **ALWAYS** include these mandatory artifacts in the story plan:
  ...
  - [ ] Include verification expectations for each story when the story changes behavior, contracts, or user-visible outcomes
  - [ ] Map personas to relevant user stories

### Story Verification Rules

- 각 user story의 acceptance criteria는 가능한 경우 테스트 가능한
  Given/When/Then 또는 관찰 가능한 결과 문장으로 작성한다.
- 각 story는 이후 Workflow Planning, Code Generation, Build and Test에서
  추적할 수 있도록 `Verification Expectations`를 포함할 수 있다.
- `Verification Expectations`에는 필요한 테스트 레벨, 자동화 테스트 필요
  여부, 수동 검증만 가능한 경우의 사유를 적는다.
- 테스트가 필요 없다고 판단한 story는 N/A 사유를 명시한다.

### Story Verification Template
...
- **Required Test Evidence**: [Expected test class, scenario, or assertion]
- **N/A Rationale**: [Only when no test is required]
```
